...
<div class="d-sm-flex align-items-center justify-content-between mb-3 flex">
    <h2 class="h3 mb-0 text-gray-800 line-1 size-1-3-rem">Văn phòng</h2>
    <a class="add-iteam" href="{{route('office.create')}}"><button class="btn-success form-control" type="button"><i class="fa fa-plus" aria-hidden="true"></i> Thêm mới</button></a>
</div>
            ...
            { 
                ?>
                    <tr id="office">
                        <td>{{$val->id}}</td>
                        <td>
                            <a href="{{route('office.edit',[$val->id])}}">{{$str}}{{$val->name}}</a>
                        </td>
                        <td>{{$val->updated_at}}</td>
                        <td style="display: flex;">
                            <a href="{{route('office.edit',[$val->id])}}" class="mr-2"><i class="fas fa-edit" aria-hidden="true"></i></a>
                            <form action="{{route('office.destroy', [$val->id])}}" method="POST">
                              @method('DELETE')
                              @csrf
